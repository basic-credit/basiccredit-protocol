...
contract BasicCredit is ReentrancyGuard {
	...
	mapping(address => mapping(address => uint)) public borrowedAmount;
	...
	function DepositTokens(address _tokenaddress, uint _amount) public {
		...
		depositBalance[_tokenaddress][msg.sender] = depositBalance[_tokenaddress][msg.sender] + _amount;
	...
	function setCreditLimit(address _tokenaddress, address _borroweraddress, uint _amount) public {
		...
		creditLimit[_tokenaddress][_borroweraddress] = _tokenaddress;
	...
	function BorrowTokens(address _tokenaddress, uint _amount) public {

		require(_amount > 0, "Amount cannot be 0");
		require(whitelistedToken(_tokenaddress) == true, "Only whitelisted tokens can be borrowed");

		pool_balance = IERC20(_tokenaddress).balanceOf(address(this))

		require(pool_balance >= _amount, "Borrowing Suspended temporarily");

		limit = creditLimit[_tokenaddress][msg.sender];
		...
		borrowedAmount[_tokenaddress][msg.sender] = borrowedAmount[_tokenaddress][msg.sender] + _amount;
